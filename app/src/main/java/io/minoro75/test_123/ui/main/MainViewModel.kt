...
class MainViewModel @ViewModelInject constructor(
	...
	init {
		...
		val p24Format = SimpleDateFormat(
            "dd.MM.yyyy",
            Locale.getDefault()
        ) //p24 uses dd.mm.yyyy format in requests
	...
	fun fetchRates() {
		viewModelScope.launch {
			try {
				_p24Rates.value = Resource.success(
                    data = exchangeRatesRepository.getP24ExchangeRates(
                        _dateP24.value!!
                    )
                )

				_nbuRates.value = Resource.success(
                    data = exchangeRatesRepository.getNbuExchangeRates(
                        _dateNbu.value!!
                    )
                )
			} catch (ex: Exception) {
